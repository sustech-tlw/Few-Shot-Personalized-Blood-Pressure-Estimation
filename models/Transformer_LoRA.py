...
import models.Transformer_block_LoRA as Transformer_block_LoRA
...
class Transformer(nn.Module):
    ...
    def __init__(self, sequence_length=1250, patch_length=50, stride_length=50, in_c=1, dim_encoder=64, depth=8,
                 ...
                 global_pool='mean', head_type='nonLinear', num_classes=1, channel_num=2, lora_r=4, lora_alpha=1.0,
        ...
        self.blocks = nn.Sequential(*[
            Transformer_block_LoRA.Transformer_Block_LoRA(
                d_model=dim_encoder, num_attention_heads=num_head, mlp_ratio=mlp_ratio, bias=bias,
                drop_ratio=drop_ratio, attn_drop_ratio=attn_drop_ratio, norm_layer=norm_layer, act_layer=act_layer, lora_r=lora_r, lora_alpha=lora_alpha
        ...
        if self.auxiliary_task:
            ...
            self.decoder_blocks = nn.Sequential(*[
                models.Transformer_block.Transformer_Block(
...
def Transformer_LoRA_model(channel_num):
    return Transformer(sequence_length=1250, patch_length=50, stride_length=50, in_c=1,
                       dim_encoder=512, depth=8, num_head=8, mlp_ratio=4, drop_ratio=0., attn_drop_ratio=0.,
                       bias=True, norm_layer=None, act_layer=None, global_pool='attention_pool',
                       head_type='nonLinear', num_classes=1, channel_num=channel_num, lora_r=8, lora_alpha=16,
                       auxiliary_task=False, dim_decoder=256, decoder_depth=3, decoder_num_head=8)
